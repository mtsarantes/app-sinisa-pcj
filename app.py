...
try:
    # AQUI ESTÁ A CORREÇÃO PARA O RENDER: Usamos o nome do ficheiro diretamente
    ...
    # Renomeando as colunas que usamos para nomes simples e fáceis
...
def health_check():
    if DATA_LOAD_ERROR: return jsonify({"status": "ERRO", "mensagem": DATA_LOAD_ERROR}), 500
    if df_dados.empty: return jsonify({"status": "ERRO", "mensagem": "DataFrame está vazio."}), 500
    return jsonify({"status": "OK", "mensagem": f"{len(df_dados)} linhas de dados carregadas."})
